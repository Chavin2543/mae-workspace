...
if dirty:
    ...
    sys.stderr.write(
        ...
        "Repo rule: for a finished task, add a short WORKLOG.md entry (date, "
        "what was done, files touched), commit all work with a descriptive "
        "message, and push (git push -u origin <current branch>) before "
        "finishing. If some files are scratch/temp, delete them (they belong "
        "in the scratchpad, not the repo), then finish.\n"
    )
    sys.exit(2)
