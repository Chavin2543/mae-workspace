...
data["arrivals"] = {
    ...
    "mots": {"2024": row(s1, 14, 3, 12), "2025": row(s1, 15, 3, 12),
             ...
             "ytd": {"2024": s1["G18"].value, "2025": s1["G19"].value,
                     "2026": s1["G20"].value}},
    "chinese": {"2019": row(s1, 26, 3, 6), "2024": row(s1, 27, 3, 6),
                "2025": row(s1, 28, 3, 6), "2026": row(s1, 29, 3, 6),
                "ytd": {"2024": s1["G33"].value, "2025": s1["G34"].value,
                        "2026": s1["G35"].value,
                        "2019": sum(v for v in row(s1, 26, 3, 6) if isinstance(v, (int, float)))}},
    "india": {"2024": row(s1, 40, 3, 6), "2025": row(s1, 41, 3, 6),
              "2026": row(s1, 42, 3, 6),
              "ytd": {"2024": s1["G46"].value, "2025": s1["G47"].value,
                      "2026": s1["G48"].value}},
    "mideast": {"2019": row(arr, 128, 18, 6), "2024": row(arr, 129, 18, 6),
                "2025": row(arr, 130, 18, 6), "2026": row(arr, 131, 18, 6)},
    "lh_usa": {"2024": row(s1, 54, 3, 6), "2025": row(s1, 55, 3, 6),
               "2026": row(s1, 56, 3, 6)},
    "lh_eu": {"2024": row(s1, 62, 3, 6), "2025": row(s1, 63, 3, 6),
              "2026": row(s1, 64, 3, 6)},
    "lh_all": {"2024": row(s1, 69, 3, 6), "2025": row(s1, 70, 3, 6),
               "2026": row(s1, 71, 3, 6),
...
# Bangkok + Pattaya STR re-based (Jun 2026): CoStar market/submarket reports
# replace the old compset series — not comparable with the 2019/2024 rows
# above. 2025 is derived from the reports' % change columns.
import xlrd

def read_str_report(path):
    sh = xlrd.open_workbook(path).sheet_by_name("Standard Monthly")
    out = {"occ": [], "adr": [], "revpar": [], "occ_ly": [], "adr_ly": [], "revpar_ly": []}
    for r in range(6, 12):  # Jan..Jun 2026
        for met, cty, cchg in [("occ", 3, 4), ("adr", 6, 7), ("revpar", 9, 10)]:
            ty, chg = sh.cell_value(r, cty), sh.cell_value(r, cchg)
            out[met].append(round(ty, 2))
            out[met + "_ly"].append(round(ty / (1 + chg / 100), 2))
    out["ytd"] = {"occ": sh.cell_value(12, 3), "adr": sh.cell_value(12, 6),
                  "revpar": sh.cell_value(12, 9),
                  "occ_chg": sh.cell_value(12, 4), "adr_chg": sh.cell_value(12, 7),
                  "revpar_chg": sh.cell_value(12, 10)}
    return out

data["str"]["pattaya_new"] = read_str_report(
