...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .delta{display:grid;grid-template-columns:1fr 70px 70px;align-items:center;
    font-size:.79rem;padding:2.5px 0;color:var(--ink2)}
  .delta span{display:flex;align-items:center;gap:6px}
  .delta b{font-variant-numeric:tabular-nums;text-align:right;white-space:nowrap}
  .delta .dh{text-align:right;color:var(--muted);font-size:.72rem;font-weight:600}
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <p class="sub">ครึ่งปีแรก (ม.ค.–มิ.ย.) ของแต่ละปี · ลูกศร = H1 2026 เทียบ H1 2024 และ H1 2023</p>
...
function changeSection(){
  ...
  metrics.forEach(([en,th,mi,dec,unit])=>{
    ...
    const hd=document.createElement("div"); hd.className="delta";
    hd.innerHTML=`<span></span><span class="dh">vs H1 2024</span><span class="dh">vs H1 2023</span>`;
    p.appendChild(hd);
    SEGS.forEach(s=>{
      const v26=D.h1[s]["2026"][mi];
      const cell=(base)=>{
        const pct=(v26/base-1)*100, up=pct>=0;
        return `<b class="${up?"up":"dn"}">${up?"▲":"▼"} ${fmt(Math.abs(pct),1)}%</b>`;
      };
